...
#Hare we write a simple malaria transmission model that includes the
# EIP (entomological innoculation period)

#Note: here we use a delay() function to describe the EIP
# 'p_surv_lat' is the proportion of mosquitoes that survive through the EIP and become infectious
malaria_model <- odin::odin({
  ...
  # parameter values
  I_init_h <- 0.001             # proportion of human population that are infectious at start of epidemic
...
#Make a parameter list. If we choose an empty list, we just run the 
# model with the default parameter values
params <- list()

#Define the model & parameters you want to use
mod_malaria <- malaria_model$new(user = params)

#Let's run the model for 10 years
t <- seq(0, 10*365, length.out=(1+10*365))
#Run model, and generate output
y1 <- mod_malaria$run(t)
#look at the output
head(y1)
#Let's prepare the output for the human compartments
dfm <- data.frame('t'=y1[,1],'S_h'=y1[,2],'I_h'=y1[,3])

#This is the human prevalence of malaria over time
ggplot(dfm) + geom_line(aes(x = t/365, y = I_h), color = 'slateblue') + 
  theme_classic() + 
  xlab('Time (years)') + ylab('Malaria prevalence in the human population')
