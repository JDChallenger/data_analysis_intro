...
#setwd("C:/Users/jchallen/Dropbox")
...
predict.lm(mod_l4, newdata = data.frame('height2' = 150 - Hbar, 'male' = 0), se.fit = T)
predict.lm(mod_l4, newdata = data.frame('height2' = 150 - Hbar, 'male' = 0), interval = 'confidence')

predict(mod_l4, newdata = data.frame('height2' = 150 - Hbar, 'male' = 0), interval = 'confidence')
predict(mod_l4, newdata = data.frame('height2' = 150 - Hbar, 'male' = 0),
...
pred_women <- predict(mod_l4, newdata = data.frame('height2' = seq(135,170) - Hbar, 'male' = rep(0,36)),
        interval = 'confidence', level = 0.95)
...
ggplot(data = d[d$male==0,]) + theme_classic() + geom_point(aes(x=height, y=weight)) + 
  ...
  geom_ribbon(data = pred_women, aes(x = height, ymin = lwr, ymax = upr), 
